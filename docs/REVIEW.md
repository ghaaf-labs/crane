...
| `@crane/server` import alias — 776 refs / 330 files; coordinated workspace rename (package.json names + tsconfig paths + esbuild/next aliases + lockfile regen), not a text find/replace | High | `packages/server/package.json:2` (+330 files); tsconfig/esbuild/next aliases |
...
| Scheduler service (`apps/schedules`: BullMQ + node-schedule) — THIRD cut: trivial HTTP/queue shell, but job bodies are the backup engine in `@crane/server` | Medium / P3 | `apps/schedules/src/utils.ts:31` |
